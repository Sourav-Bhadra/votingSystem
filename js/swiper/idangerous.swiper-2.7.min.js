var Swiper = function(l, a) {
    ...
    f.init = function(aj, ai) {
        ...
        if (a.slidesPerView === "auto") {
            ...
            if (a.scrollContainer) {
                ...
                if (parseFloat(ak) > 0 && (!a.cssWidthAndHeight || a.cssWidthAndHeight === "height")) {
                    ap.style.width = ak + "px"
                }
                if (parseFloat(au) > 0 && (!a.cssWidthAndHeight || a.cssWidthAndHeight === "width")) {
                    ap.style.height = au + "px"
                }
                az = 0;
                f.snapGrid = [];
                ...
                for (aw = 0; aw < f.slides.length; aw++) {
                    ...
                    f.slidesGrid.push(az);
                    az += J;
                    if (parseFloat(ao) > 0 && (!a.cssWidthAndHeight || a.cssWidthAndHeight === "height")) {
                        f.slides[aw].style.width = ao + "px"
                    }
                    if (parseFloat(aq) > 0 && (!a.cssWidthAndHeight || a.cssWidthAndHeight === "width")) {
    ...
    function U(ag) {
        ...
        if (document.activeElement) {
            if (document.activeElement !== ah) {
                document.activeElement.blur()
            }
        }
        var p = ("input select textarea").split(" ");
        if (a.noSwiping && (ah) && I(ah)) {
            return false
        }
        ad = false;
        f.isTouched = true;
        w = ag.type === "touchstart";
        if (!w && "which"in ag && ag.which === 3) {
            return false
        }
        if (!w || ag.targetTouches.length === 1) {
            f.callPlugins("onTouchStartBegin");
            if (!w && !f.isAndroid && p.indexOf(ah.tagName.toLowerCase()) < 0) {
            ...
            if (a.onTouchStart) {
                f.fireCallback(a.onTouchStart, f, ag)
            }
    ...
    function aa(ah) {
        ...
        if (D) {
            if ((!a.swipeToNext && ag < f.touches.startX) || ((!a.swipeToPrev && ag > f.touches.startX))) {
                return
            }
        } else {
    ...
    f.swipeNext = function(p) {
        ...
        if (a.slidesPerView === "auto") {
            for (var ag = 0; ag < f.snapGrid.length; ag++) {
                if (-ai >= f.snapGrid[ag] && -ai < f.snapGrid[ag + 1]) {
                    ah = -f.snapGrid[ag + 1];
                    break
                }
            }
                ...
                if (a.onSlideChangeEnd) {
                    if (ah === "to") {
                        if (am.runCallbacks === true) {
                            f.fireCallback(a.onSlideChangeEnd, f, al)
                        }
                    } else {
                        f.fireCallback(a.onSlideChangeEnd, f, al)
                    }
    ...
    f.calcVisibleSlides = function(j) {
        ...
        for (var ag = 0; ag < f.slides.length; ag++) {
            ak += ah;
            if (a.slidesPerView === "auto") {
                ah = D ? f.h.getWidth(f.slides[ag], true, a.roundLengths) : f.h.getHeight(f.slides[ag], true, a.roundLengths)
            } else {
                ah = J
            }
            aj = ak + ah;
